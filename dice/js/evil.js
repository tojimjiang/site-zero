...
function randDice(guess) {
	// Evil Odd Guess
	if (guess === 'odd') {
		let preReturn = (randint(6) + 1);
		while ((preReturn % 2) === 1) {
			preReturn = (randint(6) + 1);
		}
		return preReturn;
	}
	// Evil Even Guess
	...
	// Number Guess (allow for one initial big win to hook in)
	else if (oneBigWin){
		oneBigWin = false;
	...
	// Evil Number Guess
		...
		let preReturn = (randint(6) + 1);
		while (preReturn === parseInt(guess,10)) {
			preReturn = (randint(6) + 1);
		}
		return preReturn;
	}
